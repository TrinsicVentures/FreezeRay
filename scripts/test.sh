...
cd TestApp
if xcodebuild -scheme TestApp -destination 'generic/platform=iOS Simulator' build > /dev/null 2>&1; then
...
cd TestApp
if xcodebuild test -scheme TestApp -destination 'platform=macOS' 2>&1 | grep -q "Test Suite.*passed"; then
